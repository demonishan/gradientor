...
function randomStops(count = 2 + Math.floor(Math.random() * 4)) {
...
function randomLinear() {
  return {
    type: 'linear',
    angle: 45 * Math.floor(Math.random() * 9),
    repeating: false,
...
function randomRadial() {
  ...
  return {
    ...
    radialSize: sizes[Math.floor(Math.random() * sizes.length)],
    repeating: false,
    colorStops: randomStops(),
  } as GradientConfig;
}
function randomConic() {
  return {
    ...
    conicPosition: { x: Math.floor(Math.random() * 101), y: Math.floor(Math.random() * 101) },
    repeating: false,
    colorStops: randomStops(),
  } as GradientConfig;
}
